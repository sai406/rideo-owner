...
class SingleVehicleTrack : BaseActivity(),OnMapReadyCallback {
        ...
        chatHub.on("UpdatedVehicleLiveData") { args ->
            ...
            for (i in 0..array.length() - 1) {
                ...
                try {
                    ...
                    var Imei = obj.getString("Imei")
                    if(Imei.equals(SPStaticUtils.getString(SharedKey.IMEI, ""))){
                        startCar(lat, lon, angle, date, speed, ignition, status)
                        val info = InfoWindowData(
                            SPStaticUtils.getString(SharedKey.VEHICLENUMBER, ""),
                            date,
                            speed,
                            status
                        )
